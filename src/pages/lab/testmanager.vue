      ...
      if (row) {
        ...
        axios
          .get(`${host}/testrecup/get_all_from_start/${row.start}`).then((responseC) => {
            ...
            getRouteOrder.value = `http://10.154.152.88:3001/testrecup/download/get_all_from_start/${responseC.data[0].start}`;
            getRouteOrderRemove.value = `http://10.154.152.88:3001/testrecup/remove_all_from_start/${responseC.data[0].start}`;
          });
      }
    }
